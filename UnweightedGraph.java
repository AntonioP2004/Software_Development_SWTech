...
//	                loop through all neighbors of x with loop variable i 
	                for ( int j = 0; neighbors.get(x).size() > j; j++) {
//	                	Let e be the edge between x and the neighbor; 
	                	Edge e = neighbors.get(x).get(j);
	                	if (isVisited[e.v] != true) { 
	                		parent[e.v] = x; 
...
//	                     	remove loop variable j from x's neighbor list;
	                       	neighbors.get(x).remove(j);
	                       	break;
	                	} 
